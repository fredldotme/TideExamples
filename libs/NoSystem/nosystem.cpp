...
#if __wasi__
#define SUPPORTS_DYNLIB 0
#else
#define SUPPORTS_DYNLIB 1
#include <dlfcn.h>
...
int nosystem_init() {
  ...
  for (int i = 0; i < commandsNode.size(); i++) {
    ...
    std::cout << "Adding command " << commandName << " from library "
              << d.library << " (entrypoint '" << d.entrypoint << "')"
              << std::endl;
...
#else
static ResolvedDynCommand nosystem_resolvemain(const DynCommand &) {
...
pid_t nosystem_fork() { return ++recent_pid; }
...
__nosystem_split_command(const std::string &cmd) {
...
int nosystem_executable(const char *cmd) {
  ...
  const std::vector<std::string> cmd_parts =
      __nosystem_split_command(cmd_as_std);
...
int nosystem_isatty(int fd) { return 0; }
...
int nosystem_execve(const char *pathname, char *const argv[],
                    char *const envp[]) {
...
static inline RunThreadState
__nosystem_init_state(NoSystemCommand *command,
  ...
  state.execution_thread =
      std::make_unique<std::thread>([&state, &command, &args]() {
#ifndef __wasi__
        try {
#endif
...
int nosystem_system(const char *cmd) {
  ...
  RunThreadState state = __nosystem_init_state(command, args);
